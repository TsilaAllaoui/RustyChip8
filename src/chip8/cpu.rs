...
impl Cpu {
    ...
    // Clearing screen
    fn cls(&mut self) {
        self.screen_buffer = vec![vec![0; 32]; 68]
    }
    ...
    pub fn get_screen_buffer_as_vec(&mut self) -> Vec<u32> {
        let mut tmp_buffer: Vec<u32> = vec![0; 64 * 32];
        for i in 0..32 {
            for j in 0..64 {
                tmp_buffer[i * 64 + j] = self.screen_buffer[j][i];
            }
        }
        return tmp_buffer;
    }
